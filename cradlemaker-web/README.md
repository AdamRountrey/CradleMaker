...
cores under `src/wasm/`. Rebuild them from the repository root with:
...
[`WASM_PORT.md`](WASM_PORT.md). Upstream OrcaSlicer source is fetched into an
ignored local cache and is not shipped with the web app.
...
CradleMaker is source-available under the repository-level
[CradleMaker Terms of Use](../TERMS_OF_USE.md). Personal and non-commercial use
is permitted; commercial use requires prior written permission from copyright
holder Adam Rountrey. Third-party components retain their own license terms.
